...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* Mobile dropdown: click to toggle instead of hover */
  ...
  document.querySelectorAll('.ppid-form').forEach(form => {
    form.addEventListener('submit', e => {
      /* Skip validation for pengaduan, cek-status & kontak forms — let them POST/GET normally */
      if (form.id === 'formPengaduan' || form.id === 'formCekStatus' || form.id === 'formKontak' || form.id === 'formCekKontak' || form.id === 'formPermohonan' || form.id === 'formKeberatan' || form.id === 'formCekPpid' || form.id === 'formCekPpidKeberatan') {
        const inputs = form.querySelectorAll('input[required], select[required], textarea[required]');
        let valid = true;
        inputs.forEach(inp => {
          /* Checkbox: check .checked instead of .value */
          if (inp.type === 'checkbox') {
            if (!inp.checked) {
              inp.style.outlineColor = 'var(--red-light)';
              valid = false;
            }
            return;
          }
      ...
      /* Other ppid-form forms: keep existing behavior */
      e.preventDefault();
      const inputs = form.querySelectorAll('input[required], select[required], textarea[required]');
      let valid = true;
      inputs.forEach(inp => {
